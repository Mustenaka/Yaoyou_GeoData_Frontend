<template>
  <div class="page-shell">
    <PageHeader title="Mobile 数据管理" subtitle="查看 Mobile 项目包、录入数据、配置与客户端日志的解析归集状态。" />

    <div class="page-card toolbar">
      <n-select v-model:value="filters.company_id" clearable :options="companyOptions" placeholder="企业" style="width: 210px" />
      <n-select v-model:value="filters.object_type" clearable :options="mobileObjectOptions" placeholder="数据类型" style="width: 150px" />
    ...
    <n-drawer v-model:show="detailVisible" width="680">
      <n-drawer-content :title="detail?.original_filename || 'Mobile 数据详情'">
        <n-descriptions v-if="detail" label-placement="left" bordered :column="2">
          ...
          <n-descriptions-item label="表单类型">{{ detail.form_type || '-' }}</n-descriptions-item>
...
import { objectTypeLabel, parseStatusLabel, parseStatusOptions } from '@/utils/labels'
...
const columns: DataTableColumns<ClientFileItem> = [
  ...
  { title: '项目 UUID', key: 'project_uuid', minWidth: 210, render: (row) => h('span', { class: 'mono' }, row.project_uuid || '-') },
...
async function fetchList() {
  ...
  try {
    ...
    rows.value = result.list.filter((item) => item.source_client !== 'win' && item.object_type !== 'win_result')
